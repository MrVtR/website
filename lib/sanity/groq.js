...
*[_type == "comment" && post._ref == $postId] | order(createdAt desc) {
  ...
  text,
  createdAt,
  "parent": parent._ref // ✅ Get parent comment ID
}
`;

export const postsByYearQuery = groq`
*[_type == "post"  && 
  (
    !$year ||
    (
      dateTime(publishedAt) >= dateTime(string($year)+"-01-01T00:00:00Z") &&
      dateTime(publishedAt) < dateTime(string($year+1)+"-01-01T00:00:00Z")
    )
  )
]{
  ...,
...
export const AllYearsPosts = groq`
*[_type == "post"] | order(publishedAt desc) { publishedAt }
`;
